...
df_plasma_human['DOI'] = (
    ...
    .astype(str)                      # Guard against non-string values
    .str.split('\\')                  # Split by path
...
# %%
# 1. Basic combination statistics
combo_counts = (
    df_plasma_human
...
# 3. Count unique DOIs per Surface modification
surf_doi_counts = (
    df_plasma_human
...
# 4. Merge back into combo_counts
combo_counts = (
    combo_counts
...
# %%
# 1. rule: filter combinations that meet the requirement
rule_mask = (
...
# %%
# Get unique (Core, Surface modification) combinations from df_nano_external_plasma_human
unique_combos_nano = df_nano_external_plasma_human[['Core', 'Surface modification']].drop_duplicates()

# Use the Core and Surface modification combination to identify differences
mask = ~df[['Core', 'Surface modification']].apply(
    tuple, axis=1
).isin(
    unique_combos_nano.apply(tuple, axis=1)  # Compare with unique values
)
mask
